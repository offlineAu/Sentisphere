...
class ConversationService:
    ...
    def list_conversations(
        ...
        *,
        ...
        # MariaDB does not support the "NULLS LAST" modifier generated by
        # SQLAlchemy's .nullslast(), so we keep a simple descending order.
        ...
        if counselor_user_id is not None:
            # Counselor should only see conversations where they have participated
            # (sent at least one message): messages.sender_id == counselor_user_id.
            stmt = (
                stmt.join(Message, Message.conversation_id == Conversation.conversation_id)
                .where(Message.sender_id == counselor_user_id)
            )
        if include_messages:
